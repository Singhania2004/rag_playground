...
with tab2:
    st.write(
        ...
        "then run every selected pipeline against it and compare real retrieval "
        "metrics (Recall, Precision, MRR) alongside the LLM-judged scores."
    )
    ...
    n_lexical = c1.number_input("Lexical", min_value=0, max_value=15, value=4)
    n_paraphrased = c2.number_input("Paraphrased", min_value=0, max_value=15, value=4)
    n_vague = c3.number_input("Vague", min_value=0, max_value=15, value=4)
    n_multi_hop = c4.number_input("Multi-hop", min_value=0, max_value=15, value=4)
    ...
    if gen_btn:
        ...
        if total_q == 0:
            ...
            st.success(f"Generated {len(eval_set)} questions.")
        ...
        for key in selected:
            ...
            for q in eval_set:
                result = run_pipeline_with_metrics(
                    pipeline, q["question"], expected_chunk_ids=q["expected_chunk_ids"], category=q["category"]
                )
        ...
        with st.expander("Per-question raw results"):
            st.dataframe(
                df[[
                    ...
                    "faithfulness", "answer_relevance", "context_relevance", "latency_sec",
                ]],
